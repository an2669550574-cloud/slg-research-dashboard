...
| [`docs/PUBLISHERS.md`](docs/PUBLISHERS.md) | 厂商主体/资本系调研建档：方法论 + 系统在哪表（含 `/health` 自检[带雷达覆盖率] + `/gaps` 缺口[带 days_on_chart + newcomer 回流置信信号] + `/download-leads` 下载榜早期信号 + `/itunes-artist-suggestions` 雷达覆盖建议[反解开发者账号] + `/ignores` 忽略名单 + sibling 去重 + brief 戳记折叠）+ 资本集团速览 + 命名易混淆点 |
