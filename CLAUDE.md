...
| 文件 | 用途 |
|---|---|
| [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) | 架构「为什么是这样设计的」：ST 配额体系（19/月稳态 + 公司池软预留）+ 设计系统硬约束 + 新品监测/每日 digest 机制（is_reentry / 各 TOPN 配置）。**改相关代码前必读** |
...
| [`docs/PUBLISHERS.md`](docs/PUBLISHERS.md) | 厂商主体/资本系调研建档：方法论 + 系统在哪表（含 `/api/publishers/health` 自检端点 + `/api/publishers/gaps` 缺口端点 + sibling 去重 + brief 戳记折叠）+ 资本集团速览 + 命名易混淆点 |
