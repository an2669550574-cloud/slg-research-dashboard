...
| 文件 | 用途 |
|---|---|
| [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) | 架构「为什么是这样设计的」：ST 配额体系（19/月稳态 + 公司池软预留）+ 设计系统硬约束 + 新品监测/每日 digest 机制（is_reentry / 各 TOPN 配置 / 跨市场去重 / 忽略名单过滤 / 检出日志保留）。**改相关代码前必读** |
