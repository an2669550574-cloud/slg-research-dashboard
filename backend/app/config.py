...
class Settings(BaseSettings):
    ...
    # 日成本软上限（美元）。超过则 AI 端点（素材分析 / 创意迁移 / 标签分析）拒绝
    # 新请求，防止失控烧钱。太石账号本身有 $50/天/人硬上限，本护栏须低于此值。
    # 正常人工调研用量极低（历史 prod ≈ $0），故设紧一点，异常刷量更早触顶 + 告警。
    LLM_DAILY_BUDGET_USD: float = 5.0
    # 月成本软上限（美元）：日预算的累月二道保险（挡「每天刷到接近日限、累月失控」）。
    # 0 = 不启用月度门。触顶与日预算一样 429，并推维护者群告警（当月每档一次）。
