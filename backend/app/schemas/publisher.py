...
class PublisherHealthOut(BaseModel):
    ...
    total_gp_artists: int = 0              # 已 wire 的 GP 开发者账号总数（雷达 iOS+GP 双侧，很多 SLG 只在 GP）
    entities_without_gp_artist: int = 0    # 没接 GP 雷达的主体数（多为 iOS-only / 资本方，故偏高）
...
class PublisherGapOut(BaseModel):
    ...
    days_on_chart: int = 0   # 名下最持久 app 窗口内上榜的不同天数（桶内最大）：持续上榜=真发行商，一日闪现=噪声
