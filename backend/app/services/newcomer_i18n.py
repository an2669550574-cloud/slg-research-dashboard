...
# 游戏名中译的口径（_PROMPT 与 _SUBGENRE_ONLY_PROMPT 共用，改一处两处同步）：宁可留空
# 也不硬译。领导拿着一个查不到的自造译名去搜、去跟人对话，比看英文原名更糟——所以显式
# 要求「拿不准填空字符串」，渲染层遇空回落原名。
_NAME_CN_FIELD = ('"name_cn": "该游戏的简体中文名。**优先填官方或圈内通行的译名**'
                  '（例：Puzzles & Survival→末日喧嚣、三國志 真戦→三国志战略版、'
                  'Whiteout Survival→寒霜启示录）；**拿不准就填空字符串**，不要按字面硬译'
                  '——宁可留空让人看原名，也不要造一个查不到的名字；本来就是简体中文名则原样返回"')

_PROMPT = """你是手游竞品调研助手。下面是一款游戏的应用商店信息。只输出 JSON（不要解释、不要代码围栏）：
{{"summary": "一句话简体中文，说清这是什么游戏（题材+品类+核心玩法卖点），不超过35字，不带书名号", """ + _NAME_CN_FIELD + """, "subgenre": "从下面固定列表选最贴切的一个核心玩法子品类(只填列表里的词)", "translation": "把下面的商店描述完整翻译成简体中文，保留分段，不增删内容"}}
...
_SUBGENRE_ONLY_PROMPT = """你是手游竞品调研助手。判定下面这款游戏的**核心玩法子品类**（按玩法机制，不看题材），并给出中文名。只输出 JSON（不要解释、不要代码围栏）：
{{"subgenre": "从下面固定列表选最贴切的一个(只填列表里的词)", """ + _NAME_CN_FIELD + """}}
...
def _clean_name_cn(parsed: dict | None, original: str | None) -> str | None:
    """LLM 给的游戏中文名 → 干净值 or None（None = 当作没译，渲染层回落原名）。

    按 _NAME_CN_FIELD 的口径，拿不准时 LLM 会回空串。另外两种也当没译：
    ① 不含任何汉字——LLM 把西文原名原样回给了我们，写进去只会渲染成「《X》X」
    ② 与原名完全相同——同理无信息量。
    书名号剥掉（渲染层自己加）。「宁可 None」是安全方向：留空只是看原名，写错则是误导。
    """
    raw = str((parsed or {}).get("name_cn") or "").strip().strip("《》").strip()
    if not raw or not _CJK_RE.search(raw):
        return None
    if original and raw == original.strip():
        return None
    return raw[:200]


async def classify_subgenre(name: str | None, genre: str | None,
                            description: str | None) -> tuple[str | None, str | None]:
    """给一款游戏分类玩法子品类 + 中文名 → (subgenre, name_cn)，各自可为 None。
    给 app_subgenre 存量回补复用（P1-2）。

    不翻译全文（省 token）。无描述 / mock / 无 key → (None, None)（无从判机制、不瞎猜）。
    非词表值 → subgenre=None（同 translate：不脏库、精确匹配不误命中）。name_cn 走同一次
    调用产出，零增量成本；LLM 按 _NAME_CN_FIELD 口径拿不准会回空串 → None，渲染层回落原名。
    **思考型模型（gemini preview）reasoning 计入 max_tokens**，虽只要两个短值也给 1024 留
    推理余量（#182 教训，别设太小）。
    """
    ...
    return (sg if sg in SUBGENRE_VOCAB else None), _clean_name_cn(parsed, name)
...
async def translate_pending_newcomers(cap: int | None = None) -> int:
    ...
    **忽略名单内的行跳过**（人工确认非 SLG，不值当烧 LLM）。USE_MOCK_DATA / 无 key no-op。
    ...
    for app_id, (name, genre, desc) in list(seen.items())[:lim]:
        ...
        # name_cn 同 subgenre 的哲学：为 None（LLM 按口径拿不准 / 没给 / 回的还是原名）时
        # **不写该列**，避免同 app 重译时把先前已有的有效译名抹成 NULL。
        if (name_cn := _clean_name_cn(parsed, name)) is not None:
            vals["name_cn"] = name_cn
        async with AsyncSessionLocal() as db:
            await db.execute(
