...
# 游戏中文名**不再问 LLM**（2026-07-20 抽样后移除）：即使 prompt 明写「拿不准就留空」，
# 对「没有中文区发行」的游戏 LLM 也不认为自己拿不准，照样字面直译——Tribal Wars 被译成
# 《部落战争》，而那是 Clash of Clans 的知名民间别名，比看英文原名更误导。改由
# store_cn_name 查中文区商店的实际标题（客观、一手、零 ST）。这里连字段都不再要，省 token。
...
async def classify_subgenre(name: str | None, genre: str | None,
    ...
    非词表值 → None（同 translate：不脏库、精确匹配不误命中）。**中文名不在这里产出**——
    改由 store_cn_name 查商店一手数据（见模块头注释）。
    **思考型模型（gemini preview）reasoning 计入 max_tokens**，虽只要一个词也给 1024 留
    推理余量（#182 教训，别设太小）。
    """
...
async def translate_pending_newcomers(cap: int | None = None) -> int:
    ...
    **忽略名单内的行跳过**（人工确认非 SLG，不值当烧 LLM）。USE_MOCK_DATA / 无 key no-op。
    ...
    for app_id, (name, genre, desc) in list(seen.items())[:lim]:
        ...
        # 游戏中文名**以商店一手数据为准**，LLM 只作候选：2026-07-20 抽样发现 LLM 对「没有
        # 中文区发行」的游戏会字面直译且不自知（Tribal Wars → 《部落战争》，而那是 Clash of
        # Clans 的知名民间别名，领导搜到的会是另一款游戏）。查中文区商店实际标题来校验：
        # 商店有官方中文名 → 用商店的；商店没有 → 判定该游戏无中文名，**丢弃 LLM 译名**、
        # 保留原名。零 ST（iTunes lookup / GP 页均免费）。详见 store_cn_name 模块。
        from app.services.store_cn_name import fetch_store_cn_name
        store_cn = await fetch_store_cn_name(app_id)
        if store_cn:
            vals["name_cn"] = store_cn
            if (llm_cn := _clean_name_cn(parsed, name)) and llm_cn != store_cn:
                logger.info("name_cn 以商店为准: %s → 商店 %r（LLM 曾给 %r）",
                            app_id, store_cn, llm_cn)
        async with AsyncSessionLocal() as db:
            await db.execute(
