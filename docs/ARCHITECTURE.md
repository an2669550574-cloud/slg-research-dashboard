...
- **受众剥离双渲染**（`build_daily_digest(audience=)`）：同一份检测数据渲染两遍（**零额外 ST/查询**，`send_daily_digest` 内 `_render('maintainer')` + `_render('leader')`）。leader 卡剥离维护者杂讯：**剥离【榜单异动】整段（movement=老游戏排名进退/进出 Top20，竞品监控视角，领导只看新品；#164，`movement=None` 一处过滤 → 正文/今日要闻/TL;DR 异动计数统一不含）**、跳过「待建档新厂线索」整段、新品行不拼「建议建档」尾标（`build_newcomer_lines(lead_cta=False)`）、TL;DR 不计「异动 N / 待建档 N」。领导卡只剩**新品情报**（新品上架 + 下载榜 SLG + 版本 + 新区 + 视频 + 今日要闻[非异动事件]）。
