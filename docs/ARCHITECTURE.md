...
| 次市场周期 | `SYNC_SECONDARY_INTERVAL_DAYS` | 默认 30 → **prod 覆盖为 14（双周）**；JP/KR/DE/RU 走这档（早前 7 周级，已放宽到 14 在 200 上限下留余量、省 ~26/月） |
...
**次市场错峰同步（`SYNC_SECONDARY_STAGGER`，#210，零 ST）**：cadence 门控原本让**所有**次市场共用同一个 `ordinal % interval` 模 → JP/KR/DE/RU **全挤在同一天**同步（真实现象：某个双周日 10 combo 一起跑，其余 13 天 digest 只剩 US、【榜单异动】「只有美国」）。开启后给每个次市场**国家**一个相位偏移，把它们摊到 interval 窗口内不同天：**同一国家的 ios/android 同天**（该国当日市场全景），不同国家均匀铺开（`offset = i * interval // n_countries`，出现序 i；14 天 / 4 国 → JP@0 / KR@3 / DE@7 / RU@10）。**每国仍每 interval 天同步一次、总频率与 ST 配额分毫不变**，只是日历分布从「双周一次全爆发」摊成「每隔 3-4 天轮一个国家」（日常「只有美国」的天数 13/14→10/14，多国出现频率 ×4）。纯函数无状态（相位随 `SYNC_RANKING_COMBOS` 出现序确定、跨重启一致），第一个次市场国家 phase=0（与未错峰行为对齐，兼容既有 cadence 测试）。`SYNC_SECONDARY_STAGGER=False` 退回旧的全挤同一天。代码 `scheduler._secondary_stagger_offset`。**注**：错峰只改「哪天同步」不改「多久同步一次」，故次市场 movement 仍是「上一同步日 vs 今日」的双周跨度对比（异动幅度偏大是双周节奏固有属性，非错峰引入）。
