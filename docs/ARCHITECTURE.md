...
ST API 调用受**两层约束**：公司池 3000 次/月（多团队共享，真硬上限）+ 本项目 `SENSOR_TOWER_MONTHLY_LIMIT` 次/月（本地软护栏，防本项目 bug 烧穿公司池）。两者都要看，**做账以 3000 池为基准**。

### 当前稳态：~59 次/月（2026-06 实测个人计数，占公司池 ~2%）

历史用量 ~360/月，经 PR #7 / #8 / #9 三轮压缩。后续把 US 主市场从周级提到**每日**（全市场新面孔检出要日级、不漏新品）、并把次市场从月级提到**周级**、再扩到 10 个 combo（加 DE/RU），稳态回升到 ~59/月——仍只占公司池 ~2%。**同步节奏是刻意调过的，别擅自加密**；当前水位以 `QuotaBanner` / 运维笔记为准。

| 项 | 配置 | config.py 默认 | prod 实际（`backend/.env` 覆盖） |
|---|---|---|---|
| 全集 combo | `SYNC_RANKING_COMBOS` | US/JP/KR × ios/android（6 组） | **US/JP/KR/DE/RU × ios/android（10 组）** |
| 主市场 | `SYNC_RANKING_COMBOS_PRIMARY` | `US:ios,US:android` | 同默认 |
| US 榜周期 | `SYNC_PRIMARY_INTERVAL_DAYS` | 1（每日） | 同默认 |
| 次市场榜周期 | `SYNC_SECONDARY_INTERVAL_DAYS` | 30（月级） | **7（周级）** |
| 销量 | `SALES_FETCH_INTERVAL_DAYS` | 7（周级，且仅主市场） | 同默认 |
| 公司池水位 poll | `SENSOR_TOWER_ACCOUNT_USAGE_TTL_HOURS` | 1（PR #41 后；不计配额所以可频繁） | 同默认 |
| 本地硬上限 | `SENSOR_TOWER_MONTHLY_LIMIT` | 100（低于 backfill floor 150 → 回填自动停） | 同默认 |

⚠️ **doc-vs-config 注意**：combo 集合与次市场周期由 **prod `backend/.env` 覆盖**（非 config.py 默认）；config.py 顶部部分用量注释（如「6 组 ≈19/月」）是 US 周级时代的旧账，未随提到每日同步更新——**以本表 + 实测水位为准**。回滚锚点：#8=`rollback-20260601-0255`、#9=`rollback-20260601-1143`。
...
调旋钮指引：
- 要省 US 配额：调大 `SYNC_PRIMARY_INTERVAL_DAYS`（现 1=每日；7=回到周级，新面孔检出退化成周级）。
- 要省次市场配额：调大 `SYNC_SECONDARY_INTERVAL_DAYS`（现 prod=7；30=回到月级）。
- 要恢复 JP/KR 等次市场的自动销量：把它们加进 `SYNC_RANKING_COMBOS_PRIMARY`（会同时变主市场周期）。
