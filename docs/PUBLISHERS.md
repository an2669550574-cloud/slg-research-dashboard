...
- ⚠️ 命名易混淆点（PR#81 初稿写错过，2026-06-19 已查证修正）：业内"Long Tech Network Limited" 这个海外发行 publisher 名（《Last Shelter》《Rise of Castles》出版方）**实属 [11] 龙创悦动 IM30 的海外马甲**（`long tech` alias 现挂在 [11]），**不是** [16] 龙腾简合 OneMT 的产品。早期 SEED 把 "龙腾简合 Long Tech" 当一个主体建，后来才查清两家是不同公司——种子主体已废（[10] ID 空），但"Long Tech" 这名字仍在 publisher 字段里出现、靠 alias 命中 [11]。改 [16] OneMT 主体时不要把 Long Tech 系产品挂过去。
