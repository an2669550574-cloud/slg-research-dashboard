...
| `GET /api/publishers/download-leads?days=90&limit=20` | **下载榜早期信号（待建档新厂线索）**：`market_newcomer_log` 里 chart_type=free、is_slg=false、非 reentry、genre 含 strateg 的新品，扣忽略名单、跨市场同 app 收敛+富化回填。比 grossing 缺口更早（软启动期/装机量先起）；与 digest 方案① 共用同一忽略名单。**额外跑读时归属（`_load_entity_matchers`+`resolve_entity`，与新品监测页同口径）排除已归属已建档主体的 app**——存档 is_slg 是检出时点快照、永不回写，否则先检出后建档的 app 会永远赖在「待建档」里（#168，见下方坑） | 是（顶部「📥 下载榜早期信号」折叠卡，建主体/忽略[app_id 粒度]） |
