...
- 状态:**Accepted — 1a/1b/1c 已实现、部署 HK 并上线生效（2026-06-26）；query 调优已应用；SLG 门控已加（#181，2026-07-03，见「当前进度」末条）。后续仅「置顶」去噪未做**
- 日期:2026-06-26
...
- 2026-06-27:**视频搜集 job 03:50 → 02:45 UTC + 进每日 digest**——把「新品实机视频」推进钉钉每日卡（领导免开网站）。原 job 在 digest(03:00) 之后，当天新品的视频赶不上当天卡；移到核心同步(02:30~02:38)之后、digest 之前，视频在拼卡时已就位。digest 新增【新品实机视频】段（只取非隐藏，尊重软删）。**（#162 起改内联：视频并进各新品行的动作行 `🎬 _video_seg`，删除独立段——领导反馈同一批新品名在【新品上架】和【实机视频】两段重列；视频项本就全来自当日新品、必然重名。）**
- 2026-06-27:**人工去噪改软删（迁移 0032，纯加列 `newcomer_video.hidden_at`）**——原 `DELETE /videos/{id}` 是物删，噪声样本被丢弃 → **无法回溯统计召回噪声率、也拿不到「调优/匹配类改动必须拿最糟样本+真实数据」所需语料**。改：delete 端点置 `hidden_at` 而非物删；`GET /videos` 默认 `hidden_at IS NULL`（前端 UX 与硬删一致、零前端改动），新增 `?include_hidden=true` 取回被删噪声样本。**用途**：观察召回质量 = 某 app 被删条数 / 总召回条数；停用词表的设计语料 = 历史 `hidden_at NOT NULL` 标题集。后端 481 passed；迁移可逆、回滚走纯代码（旧码忽略该列）。
- 2026-07-03:**范围修正——SLG 门控（#181）**。软删语料原为攒停用词语料，但 prod 实测 `hidden_at` 样本 = **0**（用户从未在看板去噪；语料靠人工删才积累，光等无用），停用词路线搁置。真实数据反而暴露：`sync_newcomer_videos` **没有 SLG 门控**，对**所有**检出新品搜视频——**178/400（44%）召回浪费在非 SLG 新品**（足球/扑克/纸牌/塔防）。而 ADR 背景（需求①）范围本就是「新 **SLG**」。修=搜每个 app 前加**双信号 SLG 门控**：① 实时 `is_slg(app_id, publisher)`（厂商主体索引，用 live 而非 log 存档列——存档列检出时点快照永不回写，同 #168/#171 陷阱）**OR** ② `subgenre_cn LIKE '%SLG%'`（LLM 题材分类，按 app_id 聚合——救「非追踪厂商但题材是 SLG」的真竞品如 Stronghold Kingdoms/My Lands，纯 is_slg 会误杀这 7 个）。非 SLG 跳过**不记台账**（后续接入/分类后下轮自动再评估）。**人工忽略名单先行**（`_load_ignore_keys`/`_is_ignored`，人工裁决 > 题材救回，防忽略前误写的 SLG subgenre 复活）。prod 投影 KEEP 53/DROP 32 app，无核心 SLG 漏网。**时序坑**：`subgenre_cn` 由 digest 内 `translate_pending_newcomers`(03:00) 写、视频 job(02:45) 在前 → 题材救回类首日被跳过；故 digest 在 translate 后**补一轮 drain**（台账去重零重复 YT 调用），让当日视频赶上卡。残余噪声（SLG app 内同名异物）仍待软删语料。
