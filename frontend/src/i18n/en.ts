...
export const en: Translations = {
  ...
  materialAnalysis: {
    ...
    unified: {
      ...
      modelLabel: 'Model',
      modelSonnet: 'Sonnet 4.6 (value)',
      modelOpus: 'Opus 4.8 (best)',
  ...
  productsManage: {
    ...
    matchSubgenreLabel: 'Same-lane subgenre (optional, preferred)',
    matchSubgenreNone: 'Any (no subgenre match)',
    matchSubgenreHint: 'The daily digest matches competitors by gameplay mechanic (the LLM-classified subgenre of new titles == this value), tagging hits with "⚔️ same lane as <this product>". More precise than theme keywords — "apocalypse" spans gate-runner/base-building/tower-defense subgenres. Setting this ignores the theme keywords below.',
    matchSubgenreBadge: 'Same lane',
    matchKeywordsLabel: 'Theme keywords (comma-separated, optional · fallback)',
    matchKeywordsPlaceholder: 'e.g. zombie,apocalypse,survival,丧尸',
    matchKeywordsHint: 'Used only when no subgenre is set above: the digest matches these against competitor names + Chinese summaries, tagging hits with "⚔️ same lane as <this product>". Theme words are inherently broad — prefer subgenre; pick distinctive words and avoid broad terms like "war".',
    matchKeywordsBadge: 'Theme',
  ...
  tagsManage: {
    ...
    badgeMulti: 'Multi',
    badgeSingle: 'Single',
    scopeLabel: 'Applies to products',
    scopeHint: 'Empty = universal (visible to all products); otherwise only listed products see this tag',
    scopeUniversal: 'Universal',
    scopeNGames: (n: number) => `${n} product${n === 1 ? '' : 's'}`,
    scopeSearchPlaceholder: 'Search by name / app_id',
    scopeClearAll: 'Clear all',
    optScopeTitle: 'Set product scope for this option',
    optScopeSaved: 'Scope updated',
    // Product view (S4): pick a product, batch-restrict universal tags to it
    viewByTag: 'By tag',
    viewByProduct: 'By product',
    productPickLabel: 'Product',
    productPickPlaceholder: 'Pick a product to mark its exclusive tags…',
    productViewHint: 'Check = restrict this "universal" tag to this product only (others lose access). Uncheck = back to universal.',
    restrictToProduct: 'Exclusive',
    scopeOtherProducts: (names: string) => `Other products: ${names}`,
    scopeOtherProductsHint: 'This tag is restricted to other products — edit it in "By tag" view',
    ...
    // Tag packs: group dimensions into custom categories (a view, not a partition)
    packsTitle: 'Tag Packs',
    packsHint: 'Group dimensions into big categories; the materials library can facet by pack. One dimension may belong to multiple packs',
    packAdd: 'New pack',
    packAddTitle: 'New tag pack',
    packEditTitle: 'Edit tag pack',
    packEmpty: 'No tag packs yet — click "New pack" to group dimensions into categories',
    packNameLabel: 'Pack name (required, max 20 chars)',
    packNamePlaceholder: 'e.g. Resource chain / Ad hooks',
    ...
    packAdded: 'Tag pack created',
    packUpdated: 'Tag pack updated',
  ...
  publishersManage: {
    ...
    groupSlgCount: (n: number) => `${n} SLG`,
    groupLabelLabel: 'Capital group report name',
    groupLabelPlaceholder: 'e.g. "Yuanqu Group"; blank falls back to the group head name',
    groupLabelHint: 'Membership is derived from equity relations (wholly-owned/controlling/affiliate merge; minority stakes do not). This only sets the report label — setting it on any member of the group works.',
    ...
    slgBadge: 'SLG',
    gapsTitle: (n: number) => `Research gaps (${n} unattributed high-revenue publishers)`,
    gapsHint: 'Publishers with revenue in the last 30d that no alias / app_id matches. Click "Create" to prefill name + initial alias, or "Ignore" to drop confirmed non-SLG ones.',
    gapsLoading: 'Scanning for gaps…',
    gapsEmpty: 'All high-revenue publishers in the last 30d are attributed or ignored.',
    ...
    gapsIgnore: 'Ignore',
    gapsIgnoreHint: 'Confirm this is not an SLG entity to track; drop it from gaps (restorable under "Ignored")',
    gapsIgnored: (n: number) => `${n} ignored`,
    gapsRestore: 'Restore',
    gapsRestoreHint: 'Un-ignore; it returns to gaps next scan',
    gapsIgnoredDone: (name: string) => `Ignored "${name}"`,
    gapsRestoredDone: 'Restored',
    // iTunes radar coverage suggestions
    ...
    radarFrom: (appName: string) => `from "${appName}"`,
    radarUnknownArtist: '(unnamed developer account)',
    ...
    leadsTitle: (n: number) => n === 0 ? 'Download-chart early signals' : `Download-chart early signals (${n} unarchived newcomer leads)`,
    leadsLoading: 'Loading early signals…',
    leadsHint: 'New apps on the download (free) chart with genre=Strategy not yet in the whitelist — an earlier "soft-launch newcomer" signal than grossing gaps (UA spend shows up in installs first). Review then "Create entity" to archive, or "Ignore" to dismiss. Zero ST quota. Entries the LLM classifies as clearly non-SLG are filtered out (the Play genre field is developer-set and noisy).',
    leadsGatedTitle: (n: number) => `${n} filtered by gameplay gate`,
    leadsGatedHint: 'Classified from the store description as clearly non-SLG (tower defense / idle / card / casual / city-builder / other). Unclassified entries and "match-3" are kept — the latter is where "match-3 + SLG" hybrids land. A false positive here means that app\'s gameplay classification needs fixing.',
    leadsGatedExpand: 'Show',
    leadsGatedCollapse: 'Hide',
    leadsEmpty: 'No download-chart early signals yet (newcomer leads will appear here as download-chart data accumulates).',
    leadsMarket: (country: string, platform: string) => `${country}/${platform} download`,
    leadsRank: (rank: number) => `#${rank}`,
    leadsIgnore: 'Ignore',
    leadsIgnoreHint: 'Confirm not an SLG newcomer to archive → remove from early signals (also drops from digest lead segment)',
    leadsIgnoredDone: (name: string) => `Ignored "${name}"`,
    leadsCreate: 'Create entity',
    leadsCreateHint: 'Create an entity from this publisher (prefilled as initial alias) to add this newcomer to the whitelist',
